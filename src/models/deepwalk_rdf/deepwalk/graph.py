...
class Graph(defaultdict):
  ...
  def random_walk(self, path_length, alpha=0, rand=random.Random(), start=None):
    ...
    if start:
      ...
      path = [rand.choice(list(G.keys()))] # for python 3.x, we need to use list to transform dict_key into list.
...
def load_edgelist_from_memory(edgelist, undirected=True):
  G = Graph()
  for (x, y, e) in edgelist:
    x = str(x)
    y = str(y)
    e = str(e)
    G[x].append(y)
